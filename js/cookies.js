// ===================================
// GERENCIADOR DE COOKIES E CONSENTIMENTO
// ===================================
...
/**
 * Verifica se o usuário já consentiu cookies
 */
function checkConsent() {
    const stored = localStorage.getItem(COOKIES_CONFIG.storageKey);
    return stored === 'true';
}
...
function saveConsent(consent) {
    localStorage.setItem(COOKIES_CONFIG.storageKey, consent ? 'true' : 'false');
    COOKIES_CONFIG.consentGiven = consent;

    // Atualiza estado de rastreadores (quando GTM/GA4 chegar)
    if (window.gtag) {
...
function removeBanner() {
    ...
    if (banner) {
        banner.classList.add('cookies-banner-hide');
...
function createBanner() {
    ...
    banner.innerHTML = `
	  ...
	  viewBox="0 0 468.465 468.465" xml:space="preserve">
<g>
	<path d="M389.716,155.819c-70.967-79.56-157.81-118.248-193.969-86.381c-36.144,31.861-7.939,122.22,63.008,201.797
		...
		c0.289-3.925,5.076-6.746,10.692-6.339C279.335,344.756,283.636,348.28,283.346,352.17z"/>
                ...
                <div class="cookies-text">
                    <h2 class="cookies-title">Cookies e Privacidade</h2>
                    <p class="cookies-description">
                        Usamos cookies para melhorar sua experiência em nosso site. Saiba mais detalhes em nossa 
            ...
            <div class="cookies-actions">
                <button class="cookies-btn cookies-btn-reject" type="button" aria-label="Rejeitar cookies">
                    Rejeitar
                </button>
                <button class="cookies-btn cookies-btn-accept" type="button" aria-label="Aceitar cookies">
                    Aceitar
                </button>
            </div>
        </div>
    ...
    // Event listeners
    const btnAccept = banner.querySelector('.cookies-btn-accept');
    const btnReject = banner.querySelector('.cookies-btn-reject');
...
document.addEventListener('DOMContentLoaded', () => {
    // Se já consentiu, não mostra banner
    if (checkConsent()) {
        COOKIES_CONFIG.consentGiven = true;
        return;
    }

    // Mostra banner após 1 segundo (deixa página carregar)
